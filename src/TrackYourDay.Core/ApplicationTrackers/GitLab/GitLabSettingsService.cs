    ...
    public class GitLabSettingsService : IGitLabSettingsService
    {
        ...
        public void UpdateSettings(string apiUrl, string apiKey, bool enabled, int fetchIntervalMinutes, int circuitBreakerThreshold, int circuitBreakerDurationMinutes)
        {
            ...
            if (enabled)
            {
                ...
                {
                    throw new System.ArgumentException("API URL is required when enabling GitLab integration.", nameof(apiUrl));
                }
                if (string.IsNullOrWhiteSpace(apiKey))
                {
                    throw new System.ArgumentException("API Key is required when enabling GitLab integration.", nameof(apiKey));
                }
            }
