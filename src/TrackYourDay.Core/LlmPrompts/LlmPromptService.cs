...
/// <summary>
/// Generates LLM prompts from activity data.
/// </summary>
public class LlmPromptService(
    ILlmPromptTemplateRepository templateRepository,
    ...
    ILogger<LlmPromptService> logger) : ILlmPromptService
{
    ...
    public string GeneratePrompt(string templateKey, DateOnly startDate, DateOnly endDate)
    {
        ...
        if (activities.Count == 0)
        {
            throw new InvalidOperationException(
                $"No activities found for date range {startDate:yyyy-MM-dd} to {endDate:yyyy-MM-dd}");
        }
    ...
    private IReadOnlyCollection<GroupedActivity> GetActivitiesForDateRange(DateOnly startDate, DateOnly endDate)
    {
        ...
        var activities = activityRepository.Find(new ActivityByDateRangeSpecification(startDate, endDate));
        allItems.AddRange(activities);

        var meetings = meetingRepository.Find(new MeetingByDateRangeSpecification(startDate, endDate));
        allItems.AddRange(meetings);

        var userTasks = userTaskService.GetAllTasks()
            .Where(t => t.IsCompleted &&
                       DateOnly.FromDateTime(t.StartDate) >= startDate &&
                       DateOnly.FromDateTime(t.StartDate) <= endDate);
